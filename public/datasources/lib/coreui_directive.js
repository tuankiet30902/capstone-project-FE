...
function navigationDirective() {
  ...
  function link(scope, element, attrs) {
    if (element.hasClass('nav-dropdown-toggle')) {
      scope.$watch('$root.thisroute', function(newRoute) {          
        if (scope.groupmenu.items.some(item => item.path === newRoute)) {
              element.parent().addClass('open');
              scope.groupmenu.isOpen = true
          } else {
              element.parent().removeClass('open');
              scope.groupmenu.isOpen = false;
          }
      });
  }
  ...
  function link(scope, element, attrs) {
    element.on('click', function(){
      if (element.hasClass('sidebar-toggler')) {
        ...
        angular.element('body').toggleClass('sidebar-show');
      }
